...
(function () {
  ...
  var STRINGS = {
    en: {
      ...
      downloadLead: "Available on Android, iOS, Windows, and macOS.",
      ...
      statusOpening: "Trying to open Navo…",
      statusOpenFallback:
    ...
    zh: {
      ...
      downloadLead: "支持 Android、iOS、Windows 与 macOS。",
      ...
      statusOpening: "正在打开 Navo…",
      statusOpenFallback: "若 Navo 未打开，请先下载安装，然后点「在 Navo 中打开」。",
  ...
  /** Same as the lightweight bridge page: navo://theme?d=… via location.href */
  function deepLinkForPayload(payload) {
    return "navo://theme?d=" + encodeURIComponent(payload);
  }

  function tryOpenInNavo(payload, lang, statusEl) {
    if (!payload) return;
    if (statusEl) setStatus(statusEl, t(lang, "statusOpening"), "info");
    location.href = deepLinkForPayload(payload);
  }
  ...
  function boot() {
    ...
    if (openBtn) {
      openBtn.addEventListener("click", function () {
        if (!raw || !data) return;
        tryOpenInNavo(raw, lang, statusEl);
      });
    }

    window.addEventListener("hashchange", function () {
      raw = payloadFromLocation();
      data = decodePayload(raw);
      render(lang, data, raw);
    });

    // Auto-open once (same as the bridge page). Page stays usable if app is missing.
    if (raw && data && !openAttempted) {
      openAttempted = true;
      tryOpenInNavo(raw, lang, statusEl);
      window.setTimeout(function () {
        if (statusEl) setStatus(statusEl, t(lang, "statusOpenFallback"), "ok");
      }, 1200);
    }
  }
